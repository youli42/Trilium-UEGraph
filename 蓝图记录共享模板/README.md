...
| 文件 | 对应 Trilium 笔记 | 作用 |
|---|---|---|
| `蓝图渲染模板.html` | 蓝图渲染模板 (code/text/html) | 渲染界面骨架（全高画布，无工具栏/无源码面板） |
| `蓝图渲染脚本.js` | 蓝图渲染脚本 (code/js) | 胶水逻辑：定位记录、读源码、注入引擎、全高绘制 |
...
- **交互**：界面去除工具栏（标题 / 蓝图 / 源码 / 刷新按钮），画布铺满整个渲染区；**鼠标滚轮直接缩放节点图**（已改 `render.js` 不依赖 Ctrl），Ctrl+滚轮则留给 Trilium 做页面缩放；刷新由 Trilium 渲染笔记自带。源码直接在「蓝图文本」代码笔记里编辑。
...
5. 保存，打开 render 笔记即可看到铺满全页的节点图；鼠标滚轮缩放图（Ctrl+滚轮交给 Trilium 页面缩放），刷新用 Trilium 渲染笔记自带功能。源码在「蓝图文本」代码笔记里直接编辑。
